...
const SectionCard = ({ title, subtitle, children }) => (
  <div className="rounded-2xl bg-slate-900/60 border border-slate-800/80 p-6 md:p-7 backdrop-blur-sm shadow-[0_12px_32px_rgba(2,6,23,0.18)]">
...
const NetWorthTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-slate-900 border border-slate-700 rounded-xl p-3 shadow-xl">
      ...
      {payload.map((item) => (
        <div key={item.dataKey} className="text-xs font-body">
          <span className="text-slate-400">{item.name}: </span>
          <span className="text-slate-100 font-medium">{formatCurrency(item.value, true)}</span>
        </div>
      ))}
    </div>
  );
...
export default function Dashboard({ results, userName, formData, onTwinUpdate }) {
  const {
    metrics,
    projections,
    cashFlow,
    ...
    <div className="space-y-6 animate-fade-in" id="report-content">
      ...
      {activeTab === "live_twin" ? (
        <LiveTransactionPanel
          userId={results.userId || results.user_id}
        ...
        <>
          ...
          {stressTest && (
            <SectionCard
              ...
              subtitle="Shock scenarios, resilience scoring, and recovery actions"
            >
              <StressTestPanel stressTest={stressTest} />
            </SectionCard>
          )}
          ...
          {history?.length > 0 && (
            <SectionCard
              ...
              subtitle="Recent saved snapshots with auto-triggered simulation results"
            >
              ...
              <div className="space-y-3">
                {recommendations.map((rec, index) =>
                  typeof rec === "string" ? (
                    ...
                    <EnhancedRecommendationItem key={index} recommendation={rec} />
                  )
                )}
              </div>
            </SectionCard>
          ...
          {results.explainability && (
            ...
            >
              <div className="grid md:grid-cols-3 gap-4">
                <AnalysisCard title="Goal Analysis" text={results.explainability.goal_analysis} />
                <AnalysisCard title="Debt Analysis" text={results.explainability.debt_analysis} />
                <AnalysisCard title="Liquidity Analysis" text={results.explainability.liquidity_analysis} />
              </div>
            </SectionCard>
          )}
        </>
...
function AnalysisCard({ title, text }) {
  return (
    <div className="p-4 rounded-xl bg-slate-800/40 border border-slate-700/40">
      <p className="text-slate-400 font-body text-xs uppercase tracking-wider mb-2">{title}</p>
      <p className="text-slate-200 font-body text-sm leading-relaxed">{text}</p>
    </div>
  );
}
...
function EnhancedRecommendationItem({ recommendation }) {
  ...
  const CategoryIcon = categoryIcons[(recommendation.category || "").toLowerCase()] || Lightbulb;

  return (
    <div className={`p-4 rounded-xl border ${getImpactColor(recommendation.impact)}`}>
      <div className="flex items-start gap-3">
        <span className="w-9 h-9 rounded-lg bg-slate-900/45 flex items-center justify-center shrink-0">
          <CategoryIcon className="w-4 h-4 text-slate-100" />
        </span>
        <div className="flex-1">
          <h4 className="font-semibold text-slate-100 mb-1">{recommendation.title}</h4>
          <p className="text-sm text-slate-300 mb-2 leading-relaxed">{recommendation.description}</p>
          {recommendation.action && (
            <p className="text-xs font-medium text-slate-300 bg-slate-900/40 px-3 py-1.5 rounded-lg inline-block">
              {recommendation.action}
            </p>
          )}
        </div>
